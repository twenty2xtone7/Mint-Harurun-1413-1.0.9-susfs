...
SET_ZIPNAME() {
    ...
    $BUILD_KERNEL_KSU && ROOT_SOLUTION="-KSU-Next"
    ...
    if [[ $BUILD_KERNEL_BRANCH == mainline ]]; then
        FILE_NAME="Harurun-${MINT_VERSION}.A${BUILD_ANDROID_PLATFORM}.${MINT_VARIANT}${ONEUI_VERSION}${ROOT_SOLUTION}_${BUILD_DEVICE_NAME^}.zip"
    else
        FILE_NAME="HarurunBeta-${MINT_VERSION}.A${BUILD_ANDROID_PLATFORM}.${MINT_VARIANT}${ONEUI_VERSION}-${MINT_SELINUX}${ROOT_SOLUTION}_${BUILD_DEVICE_NAME^}.${MINT_TYPE}.zip"
    fi
}
